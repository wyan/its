...
39000	IF(PRSA.NE.WAVEW) GO TO 10		
	...
	RETURN
C
39200	OFLAG1(POT)=OFLAG1(POT).OR.VISIBT	
	RAINBF=.NOT. RAINBF			
	I=245					
	IF(RAINBF) I=246			
	CALL RSPEAK(I)				
...
47000	IF(CAGESF.OR.(PRSA.NE.TAKEW)) GO TO 10	
	...
	AROOM(AROBOT)=CAGED
	OFLAG1(ROBOT)=OFLAG1(ROBOT).OR.NDSCBT	
	CTICK(CEVSPH)=10			
	RETURN
C
47200	CALL NEWSTA(SPHER,0,0,0,0)		
	RFLAG(CAGER)=RFLAG(CAGER).OR.RMUNG	
	RRAND(CAGER)=147
	CALL JIGSUP(148)			
	RETURN
C
...
48300	CAROFF=.NOT.CAROFF			
	...
	CALL RSPEAK(269)			
	OFLAG1(IRBOX)=OFLAG1(IRBOX).XOR.VISIBT	
...
49100	RFLAG(HERE)=RFLAG(HERE).OR.RMUNG	
	RRAND(HERE)=271
	CALL JIGSUP(272)			
	RETURN
C
...
51000	IF((PRSA.NE.EATW).OR.(PRSO.NE.ECAKE).OR.
	...
	OFLAG1(ROBOT)=OFLAG1(ROBOT).AND. .NOT.VISIBT	
	DO 51100 I=1,OLNT			
...
52200	IF((PRSA.NE.THROWW).OR.(PRSO.NE.RDICE).OR.(PRSI.NE.POOL))
	...
	OFLAG1(SAFFR)=OFLAG1(SAFFR).OR.VISIBT	
	RETURN
C
52300	IF((HERE.NE.ALICE).AND.(HERE.NE.ALISM).AND.(HERE.NE.ALITR))
	...
	RFLAG(HERE)=RFLAG(HERE).OR.RMUNG	
	RRAND(HERE)=281
	CALL JIGSUP(282)			
	RETURN
C
52400	IF((PRSA.NE.EATW).OR.(PRSO.NE.BLICE))
	...
	OFLAG1(ROBOT)=OFLAG1(ROBOT).OR.VISIBT	
	DO 52450 I=1,OLNT			
...
61000	IF(.NOT.ENDGMF) GO TO 45000		
	...
	IF(QOPEN(TOMB)) I=794
	CALL RSPEAK(I)
	OFLAG2(TOMB)=OFLAG2(TOMB).OR.OPENBT	
	RETURN
C
