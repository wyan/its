...
500	IF((QPREP.EQ.0).OR.(QPREP.EQ.SPREP)) DFORCE=J	 
	IF((VFLAG.AND.SDRIV).NE.0) DRIVE=J	
3000	J=NEWJ
...
D30	FORMAT(' SYNMCH- DO GWIM= ',I6)
	...
	CALL ORPHAN(-1,ACT,0,DOBJ.AND.VPMASK,0)	
	CALL RSPEAK(623)
	RETURN
C
...
C
6000	IF((VFLAG.AND.SFLIP).EQ.0) GO TO 5000	
	J=O1					
	O1=O2
	O2=J
C
5000	PRSA=VFLAG.AND.SVMASK			
	PRSO=O1					
	PRSI=O2					
...
C
	VFLAG=VVOC(OLDJ)
	J=OLDJ+1
	IF((VFLAG.AND.SDIR).EQ.0) RETURN	
	DFL1=-1					
	DFL2=-1
	IF((VFLAG.AND.SSTD).EQ.0) GO TO 100	
	DFW1=-1					
	DFW2=-1
...
100	DOBJ=VVOC(J)				
	...
	DFW2=VVOC(J+2)
	J=J+3
	IF((DOBJ.AND.VEBIT).EQ.0) GO TO 200	
	DFL1=DFW1				
	DFL2=DFW2
C
200	IF((VFLAG.AND.SIND).EQ.0) RETURN	
	IFL1=-1					
	IFL2=-1
	IOBJ=VVOC(J)
	IFW1=VVOC(J+1)
...
C
	...
	RETURN
C
500	IF((SFLAG.AND.VRBIT).EQ.0) GO TO 1000	
	IF((SFLAG.AND.VTBIT).EQ.0) GO TO 2000	
C
...
C
	IF((SFLAG.AND.VCBIT).EQ.0) GO TO 4000	
...
C
2000	IF((SFLAG.AND.VCBIT).EQ.0) GO TO 4000	
...
3700	CALL NEWSTA(OBJ,559,0,0,WINNER)		
	OFLAG2(OBJ)=OFLAG2(OBJ).OR.TCHBT	
